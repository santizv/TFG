...
void StimulusPresentationTask::OnPreflight(const SignalProperties &Input) const
{
    ...
    OptionalState("PhaseInSequence");  //NEW para registrar el estado
...
{
    mLastClassResult = inResult; //NEW 

    Target *pTarget = StimulusTask::OnClassResult(inResult);
    if (pTarget != NULL)
    {
        if (State("PhaseInSequence") == 3)//NEW
        {
        ...
        {
            bcidbg(2) << "OnClassResult: Resultado recibido pero PhaseInSequence=" << State("PhaseInSequence");//NEW
        }
    }
    return pTarget;
}
...
{
    // NEW Comprobamos si estamos en la fase final.
    if (State("PhaseInSequence") == 3)
    {
        // Si aún no se ha actualizado SelectedStimulus, forzamos la actualización.
        if (State("SelectedStimulus") == 0)
        {
            bcidbg(2) << "DoPostSequence: Reintentando clasificación con el último resultado almacenado...";
            Target* pTarget = StimulusTask::OnClassResult(mLastClassResult);
            if (pTarget != NULL)
            {
                State("SelectedStimulus") = pTarget->Tag();
                mResultAnnouncement.Present();
                mPostResultBlockCount = 0;
                bcidbg(2) << "DoPostSequence: Resultado reintentado, SelectedStimulus=" << pTarget->Tag();
            }
        }
    }//NEW
