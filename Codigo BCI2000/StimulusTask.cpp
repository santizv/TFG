      ...
      mrDisplay(Window())
{
    ...
    BEGIN_STATE_DEFINITIONS
        ...
        "TriggerFromProcessing 1 0 0 0", //NEW Trigger de proccesing
        "StimulusCodeCop   16 0 0 0", //NEW Este va a ser el que manda BCI a processing
        "StimulusTypeCop   1 0 0 0", //NEW Este va a ser el que manda BCI a processing
        "StimulusBeginCop   1 0 0 0", //NEW Este va a ser el que manda BCI a processing
        "PhaseInSequenceCop 2 0 0 0", //NEW
...
void StimulusTask::StartRun()
{
    ...
    State("TriggerFromProcessing") = 0; //NEW
    State("StimulusCodeCop") = 0; //NEW
    State("StimulusTypeCop") = 0; //NEW
    State("StimulusBeginCop") = 0;//NEW
    State("PhaseInSequenceCop") = PhaseInSequence::None;//NEW
...
void StimulusTask::Process(const GenericSignal& Input, GenericSignal& Output)
{
    ...
    // Si TriggerFromProcessing se activa NEW
    if (State("TriggerFromProcessing") == 1) {
        State("TriggerFromProcessing") = 0; // reseteo para evitar activaciones repetidas
    }//NEW

    if (State("StimulusBegin") == 1) {
        State("StimulusBegin") = 0;
    }//NEW
    ...
    while (doProgress)
    {
        ...
        case stimulus: {
            ...
            State("StimulusBeginCop") = (mBlocksInPhase == 0 && !doProgress); // may be used by EarlyOffsetExpression NEW
            doProgress |= EarlyOffset(Input, Associations()[mStimulusCode]);
            State("StimulusBeginCop") = (mBlocksInPhase == 0 && !doProgress);//NEW
        ...
        case postSequence:
                if (mInterpretMode != InterpretModes::None && mBlocksSinceStimulus >= mStimToClassDuration &&
                    !mClassResult.empty() && !mCodesPresented.empty()  && mClassResult.size() == mCodesPresented.size())//NEW mClassResult.size() == mCodesPresented.size() si es igual el numero de resultados con el numero de estimulos presentados
                {
        ...
        if (doProgress)
        {
            ...
            case preRun:
                ...
                { // Enter pre sequence phase
                    State("PhaseInSequenceCop") = PhaseInSequence::PreSequence;//NEW
            ...
            case preSequence: { // Leave pre sequence phase
                State("PhaseInSequenceCop") = PhaseInSequence::Sequence;//NEW
            ...
            case ISI: {
                ...
                { // Enter stimulus phase
                    mBlocksSinceStimulus = 0;
                    State("StimulusCodeCop") = mStimulusCode;//NEW
                    //mStimulusCodeCop = mStimulusCode;//No necesario
                    State("StimulusTypeCop") = mAssociations[mStimulusCode].Contains(AttendedTarget());//NEW
                ...
                { // Enter post sequence phase
                    State("PhaseInSequenceCop") = PhaseInSequence::PostSequence; //NEW
                    mPhase = postSequence;
                }
            }
            break;
